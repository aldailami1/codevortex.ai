...
export const AcademyModule: React.FC<AcademyModuleProps> = ({
  ...
  const [tracks, setTracks] = useState<CourseTrack[]>(() => {
    try {
      const saved = safeGetItem('cloudforge_courses_v1');
  ...
  const [progress, setProgress] = useState<UserProgress>(() => {
    try {
      const saved = safeGetItem('cloudforge_user_progress_v1');
      if (saved) return JSON.parse(saved);
    } catch (e) {
  ...
  useEffect(() => {
    try {
      safeSetItem('cloudforge_courses_v1', JSON.stringify(tracks));
    } catch (e) {
      console.error(e);
    }
  }, [tracks]);
  ...
  useEffect(() => {
    try {
      safeSetItem('cloudforge_user_progress_v1', JSON.stringify(progress));
    } catch (e) {
      console.error(e);
    }
  }, [progress]);
