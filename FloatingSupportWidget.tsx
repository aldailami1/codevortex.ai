  ...
  const [messages, setMessages] = useState<Array<{ sender: 'agent' | 'user'; text: string; time: string }>>([
    {
      sender: 'agent',
      text: isAr
        ? 'مرحباً بك في **CodeVortex**! 👋 كيف يمكن لفريق الدعم الفني والذكاء الاصطناعي مساعدتك اليوم؟'
        : 'Welcome to **CodeVortex**! 👋 How can our support team & AI assist you today?',
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fallbackText = isAr
        ? 'يسعدنا تواصلك مع **CodeVortex**! لقد تم تسجيل استفسارك كـ **تذكرة دعم فني** وحفظها في بنك التذاكر بالمنصة.'
        : 'Thank you for contacting **CodeVortex**! Your query has been logged as a **support ticket** in our platform database.';
