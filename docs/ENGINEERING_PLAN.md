...
| تضارب كاش الاسم القديم CodeVortex | مفاتيح `localStorage` قديمة (`codevortex_*`) ونصوص العلامة التجارية مختلطة | إعادة تسمية شاملة CodeVortex → CloudForge + `migrateLegacyCache()` تُنفَّذ مرة واحدة عند الإقلاع |
