  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      sender: 'ai',
      text: isAr
        ? `أهلاً بك! أنا مساعدك البرمجي المباشر في منصة CodeVortex REPL IDE. يمكنك سؤالي عن إضافة ميزات جديدة، تعديل التصاميم، كود الجافاسكربت أو إصلاح الأخطاء.`
        : `Welcome! I am your CodeVortex AI Co-Pilot. Ask me to refactor code, add new features, fix layout bugs, or write custom JavaScript.`,
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-extrabold text-sm text-white flex items-center gap-2">
              <span>{isAr ? 'المساعد البرمجي المباشر' : 'CodeVortex AI Co-Pilot'}</span>
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            </h3>
            <p className="text-[10px] text-slate-400">
              {isAr ? 'متصل بالمحرك العصري CodeVortex AST Engine v5' : 'Connected to CodeVortex AST & Neural Engine v5.0'}
            </p>
          </div>
        </div>
      </div>
        ...
        {isProcessing && (
          <div className="flex items-center gap-3">
            ...
            <div className="bg-slate-900 border border-slate-800 p-3 rounded-2xl text-xs text-cyan-400 animate-pulse">
              {isAr ? 'جاري كتابة الأكواد وتعديل الملفات...' : 'CodeVortex AI is writing code and applying changes...'}
            </div>
          </div>
        )}
      </div>
